"""
Seed the problems table with batch 5 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch5.py
"""
...
from scripts.seed_problems_data_batch5 import PROBLEMS_BATCH5
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH5)

    print(f"Seeding {total} batch 5 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH5, 1):
