"""
Seed the problems table with batch 11 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch11.py
"""
...
from scripts.seed_problems_data_batch11 import PROBLEMS_BATCH11
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH11)

    print(f"Seeding {total} batch 11 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH11, 1):
