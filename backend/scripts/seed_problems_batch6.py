"""
Seed the problems table with batch 6 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch6.py
"""
...
from scripts.seed_problems_data_batch6 import PROBLEMS_BATCH6
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH6)

    print(f"Seeding {total} batch 6 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH6, 1):
