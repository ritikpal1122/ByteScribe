"""
Seed the problems table with batch 9 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch9.py
"""
...
from scripts.seed_problems_data_batch9 import PROBLEMS_BATCH9
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH9)

    print(f"Seeding {total} batch 9 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH9, 1):
