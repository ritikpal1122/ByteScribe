"""
Seed the problems table with batch 4 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch4.py
"""
...
from scripts.seed_problems_data_batch4 import PROBLEMS_BATCH4
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH4)

    print(f"Seeding {total} batch 4 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH4, 1):
