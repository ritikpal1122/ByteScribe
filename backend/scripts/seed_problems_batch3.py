"""
Seed the problems table with batch 3 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch3.py
"""
...
from scripts.seed_problems_data_batch3 import PROBLEMS_BATCH3
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH3)

    print(f"Seeding {total} batch 3 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH3, 1):
