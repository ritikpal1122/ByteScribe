"""
Seed the problems table with batch 8 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch8.py
"""
...
from scripts.seed_problems_data_batch8 import PROBLEMS_BATCH8
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH8)

    print(f"Seeding {total} batch 8 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH8, 1):
