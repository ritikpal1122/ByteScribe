"""
Seed the problems table with batch 13 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch13.py
"""
...
from scripts.seed_problems_data_batch13 import PROBLEMS_BATCH13
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH13)

    print(f"Seeding {total} batch 13 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH13, 1):
