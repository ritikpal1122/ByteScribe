"""
Seed the problems table with batch 12 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch12.py
"""
...
from scripts.seed_problems_data_batch12 import PROBLEMS_BATCH12
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH12)

    print(f"Seeding {total} batch 12 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH12, 1):
