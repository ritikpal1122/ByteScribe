"""
Seed the problems table with batch 10 problems (with company links).

Idempotent — skips problems whose slug already exists.

Usage:
    cd backend
    python scripts/seed_problems_batch10.py
"""
...
from scripts.seed_problems_data_batch10 import PROBLEMS_BATCH10
...
async def main() -> None:
    ...
    added = skipped = 0
    total = len(PROBLEMS_BATCH10)

    print(f"Seeding {total} batch 10 problems ...\n")
    ...
    async with session_factory() as session:
        ...
        for i, p in enumerate(PROBLEMS_BATCH10, 1):
